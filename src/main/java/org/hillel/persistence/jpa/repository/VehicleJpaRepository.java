...
public interface VehicleJpaRepository extends CommonJpaRepository<VehicleEntity,Long>,
    ...
    @Query(value = "select v from VehicleEntity v where v.id between :id_from and :id_to and v.name like :name")
//      @Query(value = "select v.* from vehicle v where v.id between :id_from and :id_to and v.name like :name",
